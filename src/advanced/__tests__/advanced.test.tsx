...
describe("advanced > ", () => {
  ...
  describe("새로 추가한 유틸 함수 테스트.", () => {
    test("getRemainingStock : 남아있는 재고 확인", () => {
      render(<CartPage products={mockProducts} coupons={mockCoupons} />);
      const product1 = screen.getByTestId("product-p1");
      const product2 = screen.getByTestId("product-p2");
      const product3 = screen.getByTestId("product-p3");
      ...
      fireEvent.click(addToCartButtonsAtProduct2); // 상품2 1추가
      fireEvent.click(addToCartButtonsAtProduct2); // 상품2 2추가
      fireEvent.click(addToCartButtonsAtProduct3); // 상품3 1추가
      expect(product1).toHaveTextContent("재고: 20개");
